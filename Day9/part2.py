...
def shortestDist(start, rest, dists, cache):
    if len(rest) == 1:
        ...
        if s in cache.keys():
            ...
            dist = max([dists[start + rest[i]] + shortestDist(rest[i], rest[:i] + rest[i+1:], dists, cache) for i in range(len(rest))])
            cache[s] = dist
            return dist
...
def solve(lines):
    ...
    return max([shortestDist(rest[i], rest[:i] + rest[i+1:], dists, {}) for i in range(len(rest))])
